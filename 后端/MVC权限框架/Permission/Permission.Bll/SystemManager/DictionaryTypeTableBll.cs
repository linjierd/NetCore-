...
{
    public class DictionaryTypeTableBll : BaseSystemCenterBll<DictionaryTypeTableDb, DictionaryTypeTableBll>
    {
        /// <summary>
        /// 获取指定用户名的用户
        /// </summary>
        ...
        public DictionaryTypeTableDb GetModel(string key)
        {
            ...
            return SystemCenterDal.GetModel(c => c.dt_type_key == key);
        ...
        {
            return SystemCenterDal.Set<DictionaryTypeTableDb>().Where(m).Pager(m);
        }

        public List<DictionaryTypeTableDb> GetAllList()
        {
            return SystemCenterDal.GetAllList().ToList(); ;
        }
        public void Update(DictionaryTypeTableDb model)
        {
            DictionaryTypeTableDb tempModel = SystemCenterDal.GetModel(c => c.dt_type_key == model.dt_type_key);
