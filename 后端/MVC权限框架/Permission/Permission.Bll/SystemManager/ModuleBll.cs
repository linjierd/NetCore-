...
{
    public class ModuleBll : BaseSystemCenterBll<ModuleDb, ModuleBll>
    {
        /// <summary>
        /// 获取所有的模块
        /// </summary>
        ...
        {
            return SystemCenterDal.Set<ModuleDb>().OrderBy(c => c.order_by);
        }
        public void Update(ModuleDb model)
        {
            ModuleDb tempModel = SystemCenterDal.GetModel(c => c.module_code == model.module_code);
        ...
        {
            return SystemCenterDal.GetModel(c => c.module_code == code);
        }
        /// <summary>
        /// 获取对应url的模块
        /// </summary>
        ...
        {
            return SystemCenterDal.GetModel(c => c.action_url.ToUpper() == url.ToUpper());
        }
        ...
        public IQueryable<ModuleDb> GetModuleList(string userName)
        {
            ...
            IQueryable<string> moduleCodes = SystemCenterDal.Set<RoleModuleRelationDb>()
                .Where(c=> roleIds.Contains( c.role_id)).Select(c=>c.module_code);
            return SystemCenterDal.Where(c => moduleCodes.Contains(c.module_code) && c.module_status == (int)Model.Common.GlobalCode.CommonStatus.Active)
        ...
        public string GetModuleMacheRoleTree(int id)
        {
            ...
            if (id > 0)
            {
                List<RoleModuleRelationDb> list = SystemCenterDal.Set<RoleModuleRelationDb>()
