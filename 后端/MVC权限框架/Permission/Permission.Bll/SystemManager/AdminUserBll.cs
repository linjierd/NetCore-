...
namespace Permission.Bll.SystemManager
{
    ...
    public class AdminUserBll : BaseSystemCenterBll<AdminUserDb, AdminUserBll>
    {
        public static readonly string userCookieKey = "userCookieKey";
        public static readonly string userSessionKey = "userSessionKey";
        /// <summary>
        /// 获取指定用户名的用户
        /// </summary>
        ...
        public AdminUserDb GetModel(string userName)
        {
            ...
            return SystemCenterDal.GetModel(c => c.user_name == userName);
        ...
        {
            return SystemCenterDal.Set<AdminUserDb>().Where(m).Pager(m);
        }
        ...
        public LoginUserViewModel IniLogin(AdminUserDb user)
        {
            if (user != null)
            {
                ...
                System.Web.HttpContext.Current.Session[userSessionKey] = loginUserViewModel;
        ...
        {
            if (System.Web.HttpContext.Current.Session[userSessionKey] != null)
            {
                return (LoginUserViewModel)System.Web.HttpContext.Current.Session[userSessionKey];
            }
            else
            {
        ...
        {
            System.Web.HttpContext.Current.Session[userSessionKey] = null;
        ...
        /// <param name="login"></param>
        /// <returns></returns>
        public static bool LoginUserIsPermission(string moduleCode, LoginUserViewModel login)
        {
            if (login.PermissionList != null && login.PermissionList.Count > 0)
            {
                if (login.PermissionList.FirstOrDefault(c => c.module_code.ToUpper() == moduleCode.ToUpper()) != null)
                {
                    return true;
                }
            }
          ...
          List<AdminUserRoleRelationDb> aurrList= GetAdminUserRoleRelationList(roleIds, model.user_name);
            using (var db = SystemCenterDal.DbContext)
            {
                using (var dbContextTransaction = db.Database.BeginTransaction()) //起一个事务
                {
                    try
                    {
                        SystemCenterDal.EditData(model);


                        SystemCenterDal.DbContext.Database.ExecuteSqlCommand(" delete from admin_user_role_relation where user_name=@p0;",
                                                      model.user_name);
                      //  PermissionDal.Set<AdminUserRoleRelationDb>().RemoveRange(aurrList);
                        if (aurrList != null && aurrList.Count > 0)
                        {

                            SystemCenterDal.AddList(aurrList);
                        }
                        db.SaveChanges();
        ...
        public void UpdateUserPassWord(string userName, string passWord)
        {
            ...
            AdminUserDb user = SystemCenterDal.GetModel(c => c.user_name == userName);
        ...
        {
            AdminUserDb model = SystemCenterDal.GetModel(c => c.user_name == user.user_name);
